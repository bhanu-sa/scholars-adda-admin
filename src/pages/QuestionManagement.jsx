...
const ExamManagement = () => {
  ...
  const handleUpdateQuestion = async (data) => {
    try {
      // Map answers (indices) back to option IDs
      const question = selectedQuestion;
      const answers = data.answers.map(idx => question.options[idx]?.id);
      const options = data.options;
      // Prepare translations: encode as base64 if needed by backend
      let translations = data.translations;
      if (translations && typeof translations !== 'string') {
        translations = btoa(unescape(encodeURIComponent(JSON.stringify(translations))));
      }
      await updateQuestion(question.ID, { ...data, answers, options, translations });
      setShowForm(false);
